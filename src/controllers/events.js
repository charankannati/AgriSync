...
const viewRequests = async (req, res)=>{
    try {
        ...
        var requests = await supplyChainContract.getPastEvents('buyEvent', { filter: { packageAddr: req.body.address }, fromBlock: 0, toBlock: 'latest' });
        requests = requests.filter((event) => {
            return event.returnValues.packageAddr === req.body.address && event.returnValues.seller === req.params.account;
        });
        res.status(200).json({
            requests
        });
...
const verifyBuyer = async (req, res) =>{
    ...
    try{
        ...
        let verificationOutput = await supplyChainContract.methods.verify(buyerAddress, messageHash, v, r, s).call({ from: req.params.account });
        console.log(verificationOutput);
        if (verificationOutput) {
            console.log('Buyer is verified successfully!');
            await supplyChainContract.methods.respondToEntity(buyerAddress, req.params.account, packageAddr, req.body.sellerSignature).send({ from: req.params.account })
            const data = await supplyChainContract.methods.getUserInfo(req.params.account).call();
            const role = data[ 'role' ];
            if (role === "1") {
            const crop = new web3.eth.Contract(Crop.abi, packageAddr);
            await crop.methods.updateProcessorAddress(buyerAddress).send({ from: req.params.account });
            message = 'Response sent to processor';
            } else if (role === "3") {
            const product = new web3.eth.Contract(Product.abi, packageAddr);
            await product.methods.updateWholesalerAddress(buyerAddress).send({ from: req.params.account });
            message = 'Response sent to wholesaler';
            } else if (role === "4") {
            const product = new web3.eth.Contract(Product.abi, packageAddr);
            await product.methods.updateDistributorAddress(buyerAddress).send({ from: req.params.account });
...
const viewResponses = async (req, res)=>{
    try {
        ...
        var responses = await supplyChainContract.getPastEvents('respondEvent', { filter: { packageAddr: req.body.address }, fromBlock: 0, toBlock: 'latest' });
