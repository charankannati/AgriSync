...
exports.wholesalerReceivedProducts = async (req, res) => {
    ...
    try {
        ...
        console.log(events);
        let processor = data[3];
        let signature = events[0]['returnValues'][3];
        let verificationOutput = await verifySignature(processor, signature);
        if(verificationOutput) {
        alert('Signature verified');
        await supplyChainContract.methods.productRecievedAtWholesaler(req.body.address, req.params.sender, processor, signature).send({from: req.params.sender})
...
exports.requestPackage = async (req,res) => {
    ...
    try {
        supplyChainContract.methods.requestProduct(req.params.sender, req.body.processor, req.body.packageAddress, signature).send({ from: req.params.sender })
        .once('receipt', async (receipt) => {
            alert('Request Made to processor!');
            console.log(receipt);
            isLoading(false);
        })
    } catch (error) {
...
exports.viewRecievedProducts = async (req, res) => {
    try {
        var productAddress = await supplyChainContract.methods.wholesalerGetAllProducts().call({
